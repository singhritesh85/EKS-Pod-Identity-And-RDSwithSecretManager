...
resource "aws_eks_addon" "pod_identity" {
  ...
  addon_version     = var.addon_version_podidentity       ### "v1.4.1-eksbuild.2"
  resolve_conflicts_on_create = "NONE"
  resolve_conflicts_on_update = "OVERWRITE"      # use resolve_conflicts_on_update for updating a kubernetes cluster
...
#######################################################################################################
# Configure Pod Identity in EKS
#######################################################################################################

resource "aws_iam_role" "pod_identity_role_s3" {
  name               = "eks-pod-identity-role-s3-${var.env}"
  assume_role_policy = file("trust-relationship-podidentity.json")
}

resource "aws_iam_role_policy_attachment" "s3_access_to_pod" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonS3ReadOnlyAccess"
  role       = aws_iam_role.pod_identity_role_s3.name
}

resource "aws_eks_pod_identity_association" "eks_podidentity_association_s3" {
  cluster_name    = aws_eks_cluster.eksdemo.name
  namespace       = "dexter"
  service_account = "dexter-sa"
  role_arn        = aws_iam_role.pod_identity_role_s3.arn
}

#######################################################################################################
# Configure secret strore csi driver in EKS
#######################################################################################################

data "aws_iam_policy_document" "aws_secretsmanager" {
  statement {
    actions = ["sts:AssumeRoleWithWebIdentity"]
    effect  = "Allow"
    ...
    condition {
      test     = "StringEquals"
      variable = "${replace(aws_iam_openid_connect_provider.eksopidc.url, "https://", "")}:sub"
      values   = ["system:serviceaccount:mederma:mederma-sa"]   ### mederma and mederma-sa are namespace and service account respectively.
    }
...
resource "aws_iam_policy" "iam_policy_podidentity_secretmanager" {
  name        = "iam-policy-secretmanager-${var.env}"
  path        = "/"
  description = "IAM Policy for AWS Secret Manager"
...
resource "aws_iam_role" "pod_identity_role_secretmanager" {
  name               = "eks-pod-identity-role-secretmanager-${var.env}"
  assume_role_policy = data.aws_iam_policy_document.aws_secretsmanager.json
}

resource "aws_iam_role_policy_attachment" "policy_attachment_to_role" {
  role       = aws_iam_role.pod_identity_role_secretmanager.name
